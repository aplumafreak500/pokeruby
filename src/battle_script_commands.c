    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (gBattleMoves[gCurrentMove].damageEffect == MOVE_PHYSICAL && !(gHitMarker & HITMARKER_x100000) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (gBattleMoves[gCurrentMove].damageEffect == MOVE_SPECIAL && !(gHitMarker & HITMARKER_x100000))
                {
...
static void atkAE_healpartystatus(void)
{
    ...
    if (gCurrentMove == MOVE_HEAL_BELL)
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            u8 dwAbility = GetMonData(&party[i], MON_DATA_HAS_HIDDEN_ABILITY);
            if (species != 0 && species != SPECIES_EGG)
            {
                ...
                if (gBattlerPartyIndexes[gBattlerAttacker] == i)
                    ...
                    ability = GetAbilityBySpecies(species, abilityBit, dwAbility);
